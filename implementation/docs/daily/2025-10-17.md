...
- Documented environment and secret upgrade process with ownership matrix and promotion workflow (`docs/runbooks/environment_secret_upgrade.md`).
- Authored staging k6 smoke post-sync runbook and split the `/api/v1/reviews/metrics` acceptance test into its own CI step.
- Exposed `/api/v1/tasks/{task_id}` status endpoint, `/api/v1/orchestrator/runs/{run_id}` detail lookup, and upgraded the streaming SSE envelope to a versioned schema.
- Expanded `/api/v1/reviews/metrics` to surface reviewer velocity stats (per-reviewer closures, median resolutions, daily throughput) and refreshed unit/endpoint coverage.
- Published consolidated endpoint documentation in `docs/api.md`, covering task status, reviewer metrics, run detail, and the versioned SSE envelope.
...
## Next Steps
1. Coordinate frontend consumption of the versioned SSE envelope and confirm reviewer dashboards ingest velocity fields.
2. Monitor CI runtime after isolating the review metrics acceptance step and tune pytest selection if variance increases.
3. Share the updated API doc with integrators and capture follow-up feedback.



## REMAINING
Phase 7 API surface and streaming upgrades are live and documented. Remaining follow-through: align the frontend/event consumers with the new schema and keep an eye on CI/runtime signals as the acceptance suites grow.